...
\begin{itemize}
    \item[] \textbf{Thanh ghi dữ liệu (Data Register):} Chứa dữ liệu cần truyền đi hoặc dữ liệu vừa nhận về từ slave.
    ...
    \item[] \textbf{Thanh ghi trạng thái (Status Register):} Cung cấp các cờ báo hiệu \texttt{ready} đang sẵn sàng.
\end{itemize}
...
% File: 5_hien_thuc_soc/5.4.4_ngoai_vi_ospi_ddr.tex

\subsection{OSPI-DDR}

Trong các hệ thống SoC hiện đại, nhu cầu về băng thông bộ nhớ mở rộng là cực kỳ cấp thiết, đặc biệt khi hệ thống cần xử lý các thuật toán mạng nơ-ron hoặc lưu trữ lượng lớn frame hình ảnh từ camera. Để giải quyết bài toán này, đề tài hiện thực hóa khối ngoại vi \textbf{Octal-SPI (OSPI)} tích hợp công nghệ \textbf{DDR (Double Data Rate)}, cho phép tối ưu hóa tốc độ truyền tải dữ liệu giữa FPGA và các chip Flash/RAM ngoại vi.

\subsubsection{Ý tưởng thiết kế}
Thành phần chính của thiết kế là module \textbf{ospi\_ddr}, sự kết hợp giữa SPI và I2C thì module này mở rộng xây dựng dựa trên cấu trúc máy trạng thái hữu hạn kết hợp với các khối I/O chuyên dụng của FPGA. Để thực hiện được việc dịch bit tại cả hai cạnh xung nhịp, tầng vật lý sử dụng các nguyên lý chuyển đổi dữ liệu tốc độ cao, đảm bảo pha của xung nhịp và dữ liệu luôn được căn chỉnh chính xác.

Hệ thống điều khiển bao gồm các khối chức năng chính: bộ chia tần số linh hoạt để tạo xung nhịp giao tiếp từ nền tảng 200 MHz, khối logic nạp/dịch dữ liệu 8-bit và bộ điều phối tín hiệu chọn chip (\texttt{csn}). Máy trạng thái điều khiển được thiết kế để quản lý các pha giao thức phức tạp của OSPI, bao gồm pha gửi mã lệnh (Instruction), pha địa chỉ (Address) và pha dữ liệu (Data), trong đó mỗi pha có thể được cấu hình chạy ở chế độ SDR hoặc DDR tùy theo đặc tính của linh kiện ngoại vi kết nối.

\subsubsection{Tích hợp hệ thống và giao diện AXI4-Lite}
Khối OSPI-DDR được nhúng vào không gian địa chỉ của vi xử lý thông qua module \textbf{OSPI\_axi\_lite\_core}. Module này đóng vai trò là một tầng cầu nối giữa giao thức bus AXI4-Lite tiêu chuẩn và logic điều khiển OSPI nội bộ. Việc tích hợp module \textbf{axi\_lite\_slave\_interface} giúp chuẩn hóa quy trình bắt tay dữ liệu, đảm bảo vi xử lý PicoRV32 có thể truy xuất bộ nhớ ngoài thông qua các lệnh nạp/lưu (\texttt{Load/Store}) đơn giản.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{5_hien_thuc_soc/image/ospi.png}
    \caption{Các thanh ghi kết nối giữa Bus AXI4-Lite và ngoại vi OSPI}
    \label{fig:i2c_fsm_logic}
\end{figure}
...
\begin{itemize}
    \item[] \textbf{Thanh ghi Dữ liệu (\texttt{Data Register}):} Đây là thanh ghi trung tâm thực hiện nhiệm vụ truyền nhận dữ liệu.
    
    \item[] \textbf{Thanh ghi Cấu hình Chế độ (\texttt{Configuration  Register}):} Thanh ghi này cho phép người dùng tùy chỉnh các thông số của transaction.
    
    \item[] \textbf{Thanh ghi Trạng thái (\texttt{Status Register}):} Cung cấp các cờ báo hiệu thời gian thực về tiến trình của bộ điều khiển vật lý.
\end{itemize}
...
% File: 5_hien_thuc_soc/5.5_video_streaming.tex
...
\subsection{Kiến trúc tổng thể của Video Pipeline}
% File: 5_hien_thuc_soc/5.5_video_streaming_intro.tex
...
\begin{itemize}
    \item[] \textbf{Tầng thu nhận dữ liệu vật lý (\textit{Capture Layer}):} Thu nhận tín hiệu từ giao diện DVP, thực hiện ghép byte và đồng bộ khung hình từ cảm biến camera.
    
    \item[] \textbf{Tầng quản lý bộ đệm (\textit{Memory Buffer Layer}):} Đóng vai trò là trung tâm lưu trữ tạm thời, sử dụng bộ điều khiển để quản lý việc truy cập vào vùng nhớ BRAM thông qua các giao dịch Burst tốc độ cao, giúp triệt tiêu sự lệch pha giữa tốc độ camera và tốc độ hiển thị.
    
    \item[] \textbf{Tầng truyền tải dữ liệu nội bộ (\textit{DMA Data Transfer Layer}):} Đây là tầng chức năng quan trọng cho phép hệ thống tự động trích xuất dữ liệu từ \textit{Frame Buffer} để truyền tải đến các tài nguyên khác trong SoC. Cơ chế này tận dụng khả năng truy cập trực tiếp bộ nhớ (\textit{Direct Memory Access}) để đẩy dữ liệu hình ảnh vào bộ gia tốc AI hoặc các kênh xử lý tín hiệu số mà không làm tiêu tốn chu kỳ xử lý của lõi PicoRV32, từ đó tối ưu hóa hiệu năng tổng thể của hệ thống.
    
    \item[] \textbf{Tầng điều khiển hiển thị (\textit{Display Controller Layer}):} Đảm bảo dữ liệu pixel được xuất ra màn hình thông qua giao diện HDMI một cách mượt mà và không bị nhiễu.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{5_hien_thuc_soc/image/videostreaming.png}
    \caption{Sơ đồ khối Video Streaming tích hợp trong SoC}
    \label{fig:video_pipeline}
\end{figure}

\subsection{Khối thu nhận hình ảnh từ cảm biến OV5640 (DVP Interface)}
Đề tài sử dụng cảm biến hình ảnh \textbf{OV5640} giao tiếp thông qua giao diện video kỹ thuật số song song \textbf{DVP} (Digital Video Port). Khối logic thu nhận thực hiện nhiệm vụ giải mã các tín hiệu định thời từ camera để trích xuất dữ liệu pixel hợp lệ.

Quy trình thu nhận dựa trên việc giám sát ba tín hiệu xung nhịp và đồng bộ:
\begin{itemize}
    \item[] \textbf{VSYNC (Vertical Sync):} Đánh dấu thời điểm bắt đầu một khung hình mới.
    \item[] \textbf{HREF (Horizontal Reference):} Xác định vùng dữ liệu pixel hợp lệ trên mỗi dòng quét.
    \item[] \textbf{PCLK (Pixel Clock):} Xung nhịp đồng bộ để lấy mẫu dữ liệu trên bus 8-bit ($D[9:2]$).
\end{itemize}

\subsection{Cơ chế quản lý bộ đệm khung hình (Frame Buffer)}
Do sự mất cân bằng về tốc độ giữa tần số lấy mẫu của camera và tần số quét của màn hình hiển thị, hệ thống sử dụng một bộ đệm khung hình (\textbf{Frame Buffer}) đóng vai trò vùng nhớ đệm trung gian. Khối này được hiện thực hóa bằng bộ nhớ \textit{BRAM} với cấu trúc hai cổng (Dual-port) độc lập.

Cơ chế vận hành của Frame Buffer bao gồm:
\begin{itemize}
    \item[] \textbf{Luồng Ghi (Write Path):} Đồng bộ với xung nhịp PCLK, thực hiện việc ghi dữ liệu pixel từ khối Capture vào bộ nhớ dựa trên địa chỉ được tạo ra từ bộ đếm tọa độ dòng/cột.
    \item[] \textbf{Luồng Đọc (Read Path):} Đồng bộ với xung nhịp hiển thị (\textit{Pixel Clock}), truy xuất dữ liệu pixel dựa trên yêu cầu từ bộ tạo định thời hiển thị.
\end{itemize}


\subsection{Điều khiển hiển thị và Giao diện HDMI (ADV7513)}
Tầng cuối cùng của Pipeline là bộ điều khiển hiển thị, chịu trách nhiệm tạo ra các tín hiệu đồng bộ chuẩn VGA/HD để giao tiếp với chip phát \textbf{ADV7513}.

Bộ tạo định thời Video thực hiện tính toán các khoảng thời gian quét bao gồm: \textit{Active Video}, \textit{Front Porch}, \textit{Sync Pulse} và \textit{Back Porch} cho cả hai chiều ngang (Horizontal) và dọc (Vertical).
\begin{itemize}
    \item[] \textbf{HSYNC/VSYNC:} Điều khiển quá trình quét dòng và quét khung hình trên màn hình.
    \item[] \textbf{Data Enable (DE):} Tín hiệu kích hoạt cho phép chip ADV7513 lấy mẫu dữ liệu pixel từ bus dữ liệu để chuyển đổi sang định dạng TMDS truyền qua cổng HDMI.
\end{itemize}

Việc cấu hình các tham số cho cảm biến OV5640 và chip ADV7513 được thực hiện thông qua khối ngoại vi \textbf{I2C} đã trình bày ở phần trước.
