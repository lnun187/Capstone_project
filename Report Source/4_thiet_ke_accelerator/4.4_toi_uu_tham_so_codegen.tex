...
\begin{enumerate}
    \item \textbf{Dung lượng bộ nhớ on-chip (BRAM):} Tổng kích thước của các Tile (bao gồm cả cơ chế Ping-Pong nhân hệ số 2) không được vượt quá dung lượng BRAM dành riêng cho từng loại dữ liệu.
    \begin{equation}
        2 \times (T_{c} \times T_{h} \times W) \times DW \le BRAM_{IFM\_MAX}
    \end{equation}
    \begin{equation}
        2 \times (T_{m} \times T_{c} \times R \times S) \times DW \le BRAM_{WGT\_MAX}
    \end{equation}
