...
\begin{enumerate}
    ...
    \item \textbf{Tài nguyên tính toán:} Số lượng channel IFMAP và channel OFMAP được tính toán song song ($T_k$) không được vượt quá số lượng mảng xử lý (PA) và số lượng đơn vị xử lý(PU) có trong một mảng xử lý (PA) vật lý có trên chip.
    \begin{equation}
        1 \le T_{k} \le N_{PA\_MAX}
    \end{equation}
    \begin{equation}
        1 \le T_{m} \le N_{PU\_MAX(PA)}
    \end{equation}
