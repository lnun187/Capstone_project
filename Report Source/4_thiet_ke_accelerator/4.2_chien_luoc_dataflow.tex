...
Thuật toán \ref{alg:ping_pong_logic} mô tả chi tiết quy trình quản lý bộ nhớ và luồng dữ liệu, minh họa rõ sự khác biệt khi xử lý Tile đầu tiên và các Tile sau.

\begin{algorithm}[H]
\caption{Cơ chế Ping-Pong Buffer xử lý Dữ liệu dôi ra}
\label{alg:ping_pong_logic}
\SetAlgoLined
\DontPrintSemicolon
\KwIn{Input Tiles, Weights}
\KwOut{DRAM (Valid Output Feature Map)}

% Khởi tạo con trỏ Buffer
Initialize pointers: $Buf_{curr} \leftarrow A$, $Buf_{next} \leftarrow B$\;

\For{$m = 0$ \textbf{to} $N_m - 1$}{
    \For{$h = 0$ \textbf{to} $N_h - 1$}{
        
        \textit{// Bước 1: Tính toán và Tích lũy (Reduction Phase)}\;
        \For{$c = 0$ \textbf{to} $N_c - 1$}{
            Load Input Tile ($T_c \times T_h$)\;
            Perform Convolution\;
            
            \uIf{$h == 0$ (First Tile)}{
                Accumulate valid rows $[0 \dots T_h-R]$ into $Buf_{curr}$\;
                Accumulate residual rows $[T_h-R+1 \dots T_h-1]$ into $Buf_{next}[0 \dots R-2]$\;
            }
            \Else{
                \textit{// Kế thừa residual từ bước trước đang nằm trong Buf\_curr}\;
                Accumulate boundary contribution into $Buf_{curr}[0 \dots R-2]$\;
                Accumulate new body rows into $Buf_{curr}[R-1 \dots T_h-1]$\;
                Accumulate new residual rows into $Buf_{next}[0 \dots R-2]$\;
            }
        }
        
        \textit{// Bước 2: Ghi kết quả và Xoay vòng (Drain \& Swap Phase)}\;
        \eIf{$h == 0$}{
            \textit{// Tile đầu chỉ có (Th - R + 1) hàng hợp lệ}\;
            Drain $Buf_{curr}$ rows $[0 \dots T_h-R]$ to DRAM\;
        }{
            \textit{// Các Tile sau hoàn thành đủ Th hàng (gồm cả phần trả nợ tile trước)}\;
            Drain $Buf_{curr}$ rows $[0 \dots T_h-1]$ to DRAM\;
        }
        
        Clear $Buf_{curr}$ (Reset for future use)\;
        \textbf{Swap pointers:} $Buf_{curr} \leftrightarrow Buf_{next}$\;
        \textit{// Sau lệnh Swap, Buf\_curr mới đã chứa sẵn dữ liệu dôi ra (Residual) cho vòng lặp h+1}\;
    }
}
\end{algorithm}
...
\begin{figure}[H]
    ...
    % --- Hàng 1: Trạng thái xử lý Tile Hk ---
    \begin{subfigure}[b]{0.8\textwidth}
        \centering
        % Thay đường dẫn ảnh thực tế của bạn vào đây
        \includegraphics[width=\linewidth]{image/4/buffer_hinh_a.png}
        \caption{Trạng thái xử lý Tile $H_k$: Valid vào Buffer A, Residual vào Buffer B}
        \label{fig:ping_pong_state_k}
    \end{subfigure}
    
    \par\bigskip % Tạo khoảng cách dọc giữa 2 hình
    
    % --- Hàng 2: Trạng thái xử lý Tile Hk+1 ---
    \begin{subfigure}[b]{0.8\textwidth} 
        \centering
        % Thay đường dẫn ảnh thực tế của bạn vào đây
        \includegraphics[width=\linewidth]{image/4/buffer_hinh_b.png}
        \caption{Trạng thái xử lý Tile $H_{k+1}$: Tích lũy vào Buffer B, Residual mới vào Buffer A}
        \label{fig:ping_pong_state_k_plus_1}
    \end{subfigure}
    
    \caption{Minh họa cơ chế Ping-Pong Buffer để xử lý dữ liệu dôi ra giữa hai Tile không gian liên tiếp. (a) Trong Pass xử lý Tile $H_k$, kết quả dôi ra được lưu chờ ở Buffer B. (b) Trong Pass xử lý Tile $H_{k+1}$, dữ liệu dôi ra từ Buffer B được cộng dồn với kết quả mới, đồng thời tạo ra dữ liệu dôi ra mới quay lại Buffer A.}
